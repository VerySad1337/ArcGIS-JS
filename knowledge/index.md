...
- `src/gis/GISMapEngine.js`
  ...
  - `annotatableLayerIds()` — `filterableLayerIds()` minus `"drawings"`. Restricted to the FeatureLayer-backed layers (the three fixed hosted layers plus any portal layers): a `GraphicsLayer` (`drawings`, `route`, `stops`, `searchResult`) has no `labelingInfo`/`labelsVisible` support in the ArcGIS JS API, so `drawings` is deliberately excluded even though it is otherwise filterable. `heat` and the other `ANALYSIS_EXCLUDED_LAYER_IDS` are already excluded via `filterableLayerIds()`.
...
- `src/components/AnalysisPanel.jsx` — the "ANALYSIS" panel-card (collapsed-by-default, same `panel-title-toggle` chevron pattern as `RoutingControlPanel`/`PortalLayerPanel`). The Buffer section (distance input + unit `<select>` — meters/kilometers/feet/miles — and an "Apply Buffer" button, disabled until a feature is selected and the distance is a positive number) always renders, regardless of `is3D`. The Slice section renders its "Start Slice"/"Stop Slice" toggle button only when `is3D` is true; otherwise it shows a "Switch to 3D view to use Slice." hint in its place — Slice, not the whole card, is what's gated.
- `src/app/ApplicationShell.jsx` — `bufferSelectedFeature(distance, unit)` wraps `engine.bufferSelectedFeature` (passing `showToast` as `msg`) and refreshes layers, since a successful buffer adds a graphic to the already-tracked `drawings` layer. `toggleSlice()` calls `engine.startSlice`/`stopSlice` and mirrors `engine.isSliceActive()` into `sliceActive` state, passed to `AnalysisPanel` so its button label follows the widget's real state rather than assuming a call succeeded. `toggleViewMode` also resets `sliceActive` to `false` whenever a 2D/3D switch happens, since `detachFromView` (called synchronously before the switch) always tears the widget down.

**Note on naming:** this "ANALYSIS" card is unrelated to the Filter & Aggregate System's `runAnalysis`/`getLayerAggregate` methods and the now-superseded `AnalysisPanel.jsx` mentioned in that section's "Superseded" note — that was a filter/aggregate UI later folded into `LayerControlPanel`; this is a new, separately-scoped component reusing the same filename and card-title convention for a different purpose (3D geometry tools, not attribute filtering/statistics).
