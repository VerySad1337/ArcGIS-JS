...
- **Renderer configuration** — MRT and heatmap layers carry renderer configuration (simple renderers for MRT stations/lines, a heatmap renderer with color stops and intensity bounds) defined at layer construction time inside the engine.
- **UI exposure** — `getLayers()` projects internal layer state into a plain list consumed by `ApplicationShell` and layer control UI, keeping the ArcGIS layer objects themselves out of the component tree.
