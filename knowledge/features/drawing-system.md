...
- **Attribute Persistence Is Session-Only** – `saveDrawings` (GeoJSON export) still writes empty `properties` objects; attribute values entered via the edit panel are not included in the export, and `drawingFields`/graphic attributes are lost on reload. `saveDrawings`/`uploadGeoJSON` are deliberately left as-is (a portable, drawings-only GeoJSON round-trip is still useful on its own, e.g. for opening in another GIS tool) rather than being extended to also capture attributes — `GISMapEngine.saveProjectState`/`loadProjectState` (see `knowledge/index.md`'s Project Persistence section) is the full-fidelity alternative: it captures drawings *with* their attributes, alongside every other layer's styling/filters/visibility, as part of a whole-session save/load rather than a drawings-only export.
