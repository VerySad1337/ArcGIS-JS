...
- **Runtime styling** — `setLayerStyle(id, { color, borderWidth, outlineColor, symbolType, markerStyle, lineStyle, fillStyle, size, opacity, halo, haloColor, haloSize })` lets the layer panel mutate a layer's Simple-mode symbol after construction, for layers with at least one coherent symbol to restyle (`route`, `touristAttractions`, `mrtStations`, `mrtLines`, `drawings`, portal layers). It clones the existing renderer/graphic symbol and reassigns it (via `SymbolRenderers.js`'s `applyExtendedSymbolStyle`) — the same clone-then-reassign pattern `updateHeatmapIntensity` already uses for the heatmap renderer — rather than introducing a second renderer-mutation mechanism. `getLayers()` surfaces each stylable layer's symbols as a `styleGroups` array (via `symbolToStyleGroup`, read from each layer's persisted simple-base field rather than its live renderer — see below) so the panel can initialize its inputs; `drawings` can yield multiple groups (one per distinct point/line/polygon symbol type actually present in the shared `drawLayer`), while the single-renderer layers always yield exactly one. Layers without a well-defined symbol (`stops`, `heat`) return an empty `styleGroups` and are not stylable from the panel. `outlineColor` only applies to `simple-fill` (polygon) groups, and `symbolType` scopes a `drawings` update to graphics of just that geometry type.
- **Advanced renderers and halo** — a layer (or, for `drawings`, one style group) can additionally be switched from its Simple symbol to a generated Unique Values or Class Breaks renderer, and a FeatureLayer-backed/portal simple-marker symbol can carry a two-layer CIM "halo" composite. Both are persisted independently of the Simple-mode base field (`layerRenderers` / `haloState` Maps, the same source-of-truth role `layerFilters`/`layerAnnotations` play for their systems) and are layered on top of it at seed/reattach time by `resolveSeedRenderer`, so a Simple-mode edit, an advanced renderer, and a halo can never corrupt each other's persisted state even though only one visual combination (Simple+halo, or an advanced renderer alone) is ever live on the layer at once. See `knowledge/index.md`'s "Advanced Renderer System" for the full scope calls (12-value cap, halo excluded from `drawings`/`route`, drawings' clear-has-no-snapshot asymmetry).
