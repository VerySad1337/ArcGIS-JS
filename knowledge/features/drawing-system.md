...
- `GISMapEngine.deleteSelectedFeature()` removes the selected graphic from `drawLayer` outright and fires `onDrawingsChanged` so the layer panel re-reads its style groups. Also in-memory only.
...
- **No Undo/Redo, and no geometry editing after the fact** – SketchViewModel's built‑in update/reshape tools are not exposed, so a completed graphic's geometry cannot be changed. A single graphic *can* be deleted after creation (2026-08): clicking it opens `FeatureAttributesPanel`, whose *Delete Feature* control routes to `GISMapEngine.deleteSelectedFeature()` — an in-memory `drawLayer.remove` for a graphic still on the local scratch layer, or an `applyEdits({ deleteFeatures })` for one already pushed to a hosted/portal target. See `knowledge/features/feature-attributes.md`. There is still no multi-select or "clear all drawings" action.
