...
- `nginx.conf` (repo root) – sets the caching policy the SPA needs (see "Stale-deploy 404s on 2D/3D toggle" below): `/assets/*` (Vite's content-hashed JS/CSS chunks) get `Cache-Control: public, max-age=31536000, immutable`; `index.html` and `env-config.js` both get `Cache-Control: no-cache` so they always revalidate. It also enables `gzip` (JS/CSS/JSON/SVG only — the fonts and ArcGIS wasm/image assets are already-compressed formats) and `http2 on`, both of which the stock `nginx:alpine` config leaves off; see `knowledge/features/performance.md` for the measurements and for why `gzip_vary` is load-bearing against the `immutable` policy above. `location / { try_files $uri $uri/ /index.html; }` is the standard SPA fallback (the app has no client-side router today, but this costs nothing and avoids a 404 if one is added later). The server block listens on `443 ssl` rather than `80`.
