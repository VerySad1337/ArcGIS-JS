...
- `src/gis/GISMapEngine.js` — `layerFilters` (Map of id -> filter definition) is the actual source of truth for "is this layer filtered right now"; a hosted layer's `definitionExpression` and the drawings layer's per-graphic `visible` flag are just the two different mechanisms used to *apply* whatever is stored there, both re-derived from the map, never the other way around.
  ...
  - `runAnalysis(ids, options)` — runs `getLayerAggregate` over several layers and combines them into a grand total. Layers can have entirely different schemas, so `field` is one name applied uniformly across every selected layer (the panel only offers spelling suggestions per layer, not a per-layer field picker); the total's `sum`/`min`/`max` simply combine whatever each layer actually returned, and `avg` is re-derived from the combined `sum`/`count` rather than averaging the per-layer averages, which would misweight layers with different feature counts.
  - `getLayers()` now includes `filterable: true` and `filterDescription` (from `getLayerFilterDescription(id)`, or `null` when inactive) for every filterable layer's entry, so both `LayerControlPanel` and `AnalysisPanel` can show current filter state without separate plumbing.
- `src/components/AnalysisPanel.jsx` — collapsed-by-default sidebar panel (same occasional-action pattern as `PortalLayerPanel`), rendered below `LayerControlPanel`. A Filter/Aggregate/Both mode toggle plus a layer checklist; selecting a layer lazily loads its field schema. In Filter (or Both) mode, each selected layer gets its own condition-row builder (field → operator → value, AND/OR combination, Apply/Clear) — filtering is per-layer, not one global expression, since layers share no common schema. In Aggregate (or Both) mode, one shared numeric-field input (with a datalist of suggestions drawn from the selected layers' schemas) plus sum/avg/min/max checkboxes drive `onRunAnalysis`, rendering a results table with one row per selected layer and a total row.
- `src/app/ApplicationShell.jsx` — `getLayerFields`/`applyLayerFilter`/`clearLayerFilter`/`runAnalysis` wrappers around the engine calls, following the same refresh-layers-and-toast pattern as `updateLayerStyle`/`addPortalLayer`.
